...
export const MembersContext: React.Context<IMember[]> = React.createContext<IMember[]>([]);

export default function App() {
  const [page, setPage] = React.useState<Page>(PAGES_ORDER[0]);
  const [members, setMembers] = React.useState<IMember[]>([]);

  const currPageIndex: number = PAGES_ORDER.findIndex((pageI) => pageI === page);
    ...
    <MembersContext.Provider value={members}>
      <div id="app">
        ...
        <div id="app-content">
          ...
          {page === Page.MEMBERS &&
            <Members onSetMembers={setMembers} />
          }
          {page === Page.COHESION &&
            <Cohesion onSetMembers={setMembers} />
          }
          {page === Page.SPEEDBOAT &&
            <Speedboat />
          }
        ...
        <Footer
          currPage={page}
          onPrev={currPageIndex !== 0
            ? () => setPage(PAGES_ORDER[currPageIndex - 1])
            : undefined
          }
          onNext={currPageIndex !== PAGES_ORDER.length - 1
            ? () => setPage(PAGES_ORDER[currPageIndex + 1])
            : undefined
          }
        />
