...
export default function LifePoints() {
  ...
  const handleClick = useCallback(
    () =>
      setLifePoints((prevLifePoints) =>
        prevLifePoints > 0 ? prevLifePoints - 1 : 0
      ),
    []
  );
  const handleDoubleClick = useCallback(() => setLifePoints(HEART_COUNT), []);

  const lifePointHearts: React.ReactNode[] = Array.from(Array(HEART_COUNT)).map(
    (_, index) => <Heart key={index} filled={index < lifePoints} />
  );
